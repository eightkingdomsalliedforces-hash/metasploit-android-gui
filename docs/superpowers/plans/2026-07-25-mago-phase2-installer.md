...
- [x] Add the three missing stage actions to Kotlin and both schemas.
- [x] Add shared JSON escaping, state paths, lock acquisition and process helpers.
- [x] Dispatch every allowlisted action to a fixed script path; reject unknown action and invalid operation ID.
- [x] Run `bash termux-bridge/tests/contract_test.sh` and confirm unknown actions still return exit code `64`.
...
- [x] Update Termux package indexes with non-interactive `pkg`.
- [x] Install the fixed dependency allowlist required by Ruby, PostgreSQL and native gems.
- [x] Clone or fast-forward the official Rapid7 repository, install the Bundler version from `Gemfile.lock`, and run production `bundle install`.
- [x] Verify `msfconsole`, `msfrpcd` and `msfdb` before reporting success.
- [x] Add repair and update actions without destructive database deletion.
...
- [x] Initialize the Metasploit database with `msfdb --component database --use-defaults init` only when configuration is absent.
- [x] Generate 32-byte RPC credentials with `openssl rand`, store them at mode `600`, and never pass them as process arguments.
- [x] Start/stop PostgreSQL and RPC using PID/state files under `$HOME/.mago`.
- [x] Extend health data with database, repository, RPC process and exact localhost port status.
- [x] Map package, database and RPC failures to distinct Bridge exit codes.
...
- [x] Include `lib/common.sh` and every allowlisted action in the deterministic archive.
- [x] Use a temporary HOME/PREFIX and fake commands to prove repeated install and configuration are idempotent.
- [x] Verify RPC password never appears in the command line or test logs.
- [x] Rebuild the bundle and update `BridgeBundleMetadata` digest and version.
...
- [x] Encode encrypted values as versioned IV+ciphertext records.
- [x] Clear input `CharArray` after save attempts.
- [x] Replace `UnconfiguredSecretStore` in `AppContainer`.
- [x] Keep only pure record-format tests; AndroidKeyStore behavior is covered by one instrumentation smoke test later.
...
- [x] Inspect health first and skip already satisfied stages.
- [x] Run update, dependency, install, database, credential and service actions in stage order.
- [x] Persist each stage before the operation and after success/failure.
- [x] Store RPC credentials, clear the response password copy, then call RPC health/version.
- [x] Classify database and RPC errors into existing `InstallationFailureKind` values.
- [x] Add only coordinator tests for resume, database failure and RPC credential storage.
...
- [x] Remove Phase 1-only wording.
- [x] Display package, Metasploit, database, RPC and verification stages.
- [x] Document expected storage/time, Termux `allow-external-apps=true`, and recovery actions.
...
- [x] Add Phase 2 shell tests to the existing Bridge verification step.
- [x] Build the affected modules and debug APK.
- [x] Upload failure diagnostics and the successful APK artifact.
- [x] Open a PR, fix only evidenced CI failures, and merge after a green run.
