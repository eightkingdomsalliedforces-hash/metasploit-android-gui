...
class MessagePackRpcCodecTest {
    ...
    @Test
    fun `integer map keys are normalized to decimal strings for session ids`() {
        val bytes = MessagePack.newDefaultBufferPacker().use { packer ->
            packer.packMapHeader(1)
            packer.packInt(7)
            packer.packMapHeader(1)
            packer.packString("type")
            packer.packString("shell")
            packer.toByteArray()
        }

        val decoded = codec.decode(bytes) as RpcValue.MapValue

        assertThat(decoded.value).containsKey("7")
        val session = decoded.value["7"] as RpcValue.MapValue
        assertThat(session.value["type"]).isEqualTo(RpcValue.StringValue("shell"))
    }

    @Test
    fun `normalization collision between integer and string keys is rejected`() {
        val bytes = MessagePack.newDefaultBufferPacker().use { packer ->
            packer.packMapHeader(2)
            packer.packInt(1)
            packer.packString("integer")
            packer.packString("1")
        ...
        val error = try {
            codec.decode(bytes)
            null
        } catch (caught: RpcCodecException) {
            caught
        }

        assertThat(error).isNotNull()
        assertThat(error?.errorCode).isEqualTo("RPC_DUPLICATE_MAP_KEY")
    }
}
