...
set -euo pipefail
ACTION="${1:-}"; OPERATION_ID="${2:-}"; SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
...
case "$ACTION" in
  ...
  BACKUP_ENVIRONMENT|RESTORE_ENVIRONMENT)
    printf '{"schemaVersion":1,"operationId":"%s","action":"%s","success":false,"exitCode":64,"message":"Action is planned for a later phase","progress":0,"data":{}}\n' "$OPERATION_ID" "$ACTION"
    exit 64 ;;
  *)
